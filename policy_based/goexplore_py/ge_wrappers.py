...
from atari_reset.atari_reset.wrappers import (CloudpickleWrapper, MyWrapper,
                                              VecWrapper, VideoWriter, worker)
...
class GoalConVecFrameStack(VecWrapper):
    ...
    def __init__(self, venv, nstack):
        ...
        self.stacked_obs = np.zeros((venv.num_envs, ) + ob_low.shape,
...
class GoalConVecGoalStack(VecWrapper):
    ...
    def __init__(self, venv, goal_rep):
        ...
        self.stacked_obs = np.zeros((venv.num_envs, ) + new_shape,
...
class GoalExplorer:
    ...
    def choose(self, go_explore_env):
        raise NotImplementedError(
            'GoalExplorers need to implement a choose method.')


class CityDomKnowNeighborGoalExplorer(GoalExplorer):
    def __init__(self, res, param_bounds, random_exp_prob, random_explorer):
        super(CityDomKnowNeighborGoalExplorer,
              self).__init__(random_exp_prob, random_explorer)
        self.res = res
    ...
    def choose(self, go_explore_env):
        ...
        for offset in self.offsets:
            possible_neighbor = get_city_neighbor(
                go_explore_env.env, go_explore_env.last_reached_cell,
                self.param_bounds, offset)

            if possible_neighbor is not None:
                if (possible_neighbor not in go_explore_env.archive.archive
                        and possible_neighbor not in
                        go_explore_env.locally_explored):
...
class DomKnowNeighborGoalExplorer(GoalExplorer):
    def __init__(self, x_res, y_res, random_exp_prob, random_explorer):
        super(DomKnowNeighborGoalExplorer,
              self).__init__(random_exp_prob, random_explorer)
        self.x_res = x_res
        self.y_res = y_res

    def choose(self, go_explore_env):
        width = go_explore_env.env.recursive_getattr(
            'screen_width') * go_explore_env.env.recursive_getattr('x_repeat')
...
class EntropyManager:
    def __init__(self, inc_ent_fac: float, ret_inc_ent_thresh: int,
                 expl_inc_ent_thresh: int, entropy_strategy: str,
                 ent_inc_power: float, ret_inc_ent_fac: float,
                 expl_ent_reset: str, legacy_entropy: bool):
    ...
    def get_return_entropy(self, env):
        if env.current_cell in self.entropy_cells:
            self.ent_end_cell, self.ent_fac = self.entropy_cells[
                env.current_cell]
        ...
        if not (self.ret_inc_ent_fac == 0 and self.ret_inc_ent_thresh == -1):
            if self.legacy_entropy:
                deadline = env.goal_cell_info.trajectory_len * \
                    self.ret_inc_ent_fac + self.ret_inc_ent_thresh
                ...
                deadline = estimate_to_current_cell * \
                    self.ret_inc_ent_fac + self.ret_inc_ent_thresh
    ...
    def get_exploration_entropy(self, env):
        if not self.expl_inc_ent_thresh == -1:
            if self.expl_ent_reset == 'on_new_cell':
                return self._get_inc_entropy(env.expl_steps_since_new_cell,
                                             self.expl_inc_ent_thresh)
            elif self.expl_ent_reset == 'on_goal_reached':
                return self._get_inc_entropy(env.actions_to_goal,
                                             self.expl_inc_ent_thresh)
    ...
    def _get_inc_entropy(self, actions_taken, deadline):
        ...
        if self.entropy_strategy == 'fixed_increase':
            ...
            raise NotImplementedError('Unknown entropy strategy:',
...
class GoalConGoExploreEnv(MyWrapper):
    ...
    """
    def __init__(self, env: Any, archive: Any,
                 goal_representation: AbstractGoalRepresentation,
                 done_on_return: bool, video_writer: VideoWriter,
                 goal_explorer: GoalExplorer,
                 trajectory_tracker: TrajectoryTracker,
                 entropy_manager: EntropyManager, max_exploration_steps: int,
                 on_done_reward: float, no_exploration_gradients: bool,
                 game_reward_factor: float, goal_reward_factor: float,
                 clip_game_reward: bool, clip_range: Tuple[float, float],
                 max_actions_to_goal: int, max_actions_to_new_cell: int,
                 cell_reached: Callable[[Any, Any], bool],
                 cell_selection_modifier: str, traj_modifier: str,
                 fail_ent_inc: str, final_goal_reward: float):
    ...
    def _choose_return_goal(self):
        ...
        if self.cell_selection_modifier == 'prev' or self.traj_modifier == 'prev':
            ...
            trajectory = self.archive.cell_trajectory_manager.get_trajectory(
                temp_goal_cell_info.cell_traj_id,
        ...
        if self.traj_modifier == 'prev' and prev_goal_cell_rep[
                0] != goal_cell_rep:
            ...
            self.entropy_manager.entropy_cells[
                final_cell[0]] = goal_cell_rep, (chosen * 0.1)**2
            trajectory.append((goal_cell_rep, 1))
        else:
            trajectory = self.archive.cell_trajectory_manager.get_trajectory(
                self.goal_cell_info.cell_traj_id,
        ...
        if self.fail_ent_inc == 'time' or self.fail_ent_inc == 'cell':
            for i, (cell_key, time_spend) in enumerate(trajectory):
                ...
                nb_failures_above_thresh = self.archive.archive[
                    cell_key].nb_failures_above_thresh
        ...
        self.sub_goal_cell_rep = self.trajectory_tracker.reset(
            self.current_cell, trajectory, self.goal_cell_rep)
    ...
    def select_cell_from_archive(self):
        archive = self.archive.archive
        chosen_cell_key = self.archive.cell_selector.choose_cell_key(
            archive)[0]
    ...
    #print('current, goal, sub goal cells: ', self.current_cell, self.goal_cell_rep, self.sub_goal_cell_rep)
        ...
        goal = self.goal_representation.get(self.current_cell,
                                            self.goal_cell_rep,
                                            self.sub_goal_cell_rep)

        return goal

    def step(self, action: int):
        print('GoalConGoExplore step')
        # Clear cache
        ...
        if self.returning:
            ...
            traj_goals = self.trajectory_tracker.step(self.current_cell,
                                                      self.goal_cell_rep)
            ...
            self.info[
                'increase_entropy'] = self.entropy_manager.get_exploration_entropy(
                    self)
            ...
            if (self.max_actions_to_new_cell >=
                    0) and (self.expl_steps_since_new_cell >
                            self.max_actions_to_new_cell):
                done = True
        ...
        if done:
            ...
            ep_info = {
                'l': self.action_nr,
                'r': self.score,
                'nb_exploration_goals_reached':
                self.nb_exploration_goals_reached,
                'nb_exploration_goals_chosen':
        ...
        if self.clip_game_reward:
            game_reward = utils.clip(game_reward, self.clip_range[0],
                                     self.clip_range[1])
...
class GoalConSubprocVecEnv(object):
    def __init__(self, env_fns, start_method):
        ...
        self.remotes, self.work_remotes = zip(
            *[mp_context.Pipe(duplex=True) for _ in range(nenvs)])
        self.ps = [
            mp_context.Process(target=worker,
...
class SilEnv(MyWrapper):
    def __init__(self, env, trajectory_tracker, goal_representation, gamma,
                 sil_invalid):
        MyWrapper.__init__(self, env)
    ...
    def step(self, *args, **kwargs):
        if not self.sil_on:
            ...
            return self.decompress(
                self.sil_observations[current_step]
            ), self.sil_rewards[current_step], done, info
    ...
    def reset(self):
        ...
        if self.next_trajectory is None:
            ...
            self.trajectory_tracker.reset(self.next_trajectory[0][0],
                                          self.next_cell_trajectory,
                                          goal_cell_rep)
            ...
            for i, data in enumerate(self.next_trajectory):
                ...
                goal = self.goal_representation.get(cell_key, goal_cell_rep,
                                                    sub_goal_cell_rep)

                obs_and_goal = (obs, goal)

                if i == 0:
                    self.sil_initial_observation = self.decompress(
                        obs_and_goal)
    ...
    def set_sil_trajectory(self, trajectory, cell_trajectory):
        print('set sil traj: {}'.format(trajectory))
